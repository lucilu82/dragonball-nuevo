  ...
  try {
    ...
    for (const url of urls) {
      try {
        ...
        lastError = error
        console.error('❌ Error actualizando planeta en Laravel:', {
          url,
    ...
    throw createError({
      statusCode: 503,
      statusMessage: 'No se pudo contactar con Laravel para actualizar el planeta'
    })
  } catch (error: any) {
    console.error('Error general en proxy planetas.put:', error)
    ...
    throw createError({
      statusCode: 500,
      statusMessage: 'Error proxy actualizando planeta',
      data: {
        message: error?.message || String(error),
